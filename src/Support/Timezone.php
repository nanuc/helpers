...
namespace Nanuc\Helpers\Support;
...
class Timezone
{
    ...
    {
        return Arr::get(self::$timezoneList, $code);
    }

    public static function format($dateType = 'MEDIUM', $timeType = 'SHORT', $date = null, $timezone = 'Europe/Berlin')
    {
        $date = $date ?? now();

        $formatter = new IntlDateFormatter(
            app()->getLocale(),
            constant('IntlDateFormatter::' . $dateType),
            constant('IntlDateFormatter::' . $timeType),
            IntlTimeZone::createTimeZone($timezone)
        );
        return $formatter->format(Carbon::parse($date));
    }

    public static function byCode($code)
    {
        return Arr::get(array_flip(self::$timezoneList), $code);
    }
}
